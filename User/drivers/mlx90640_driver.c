...
/* Conditional printf for MLX90640 driver module */
#if (ENABLE_DRV_PRINTF && ENABLE_DRV_PRINTF_MLX90640_DRIVER)
    #define MLX90640_DRIVER_PRINTF(...)    printf(__VA_ARGS__)
#else
    #define MLX90640_DRIVER_PRINTF(...)    ((void)0)
#endif
...
int MLX90640_GetFrameData(uint8_t slaveAddr, uint16_t *frameData)
{
    ...
    while(dataReady == 0)
    {
        ...
        if(error != 0)
        {
            MLX90640_DRIVER_PRINTF("GetFrameData: Initial status read failed, error=%d\r\n", error);
            return error;
        }    
        dataReady = statusRegister & 0x0008;
    }
    
    MLX90640_DRIVER_PRINTF("GetFrameData: Data ready detected, statusReg=0x%04X\r\n", statusRegister);
        
    // Clear the data ready flag by writing to status register
    MLX90640_DRIVER_PRINTF("GetFrameData: Writing 0x0030 to clear status...\r\n");
    error = MLX90640_I2CWrite(slaveAddr, 0x8000, 0x0030);
    if(error == -1)
    {
        MLX90640_DRIVER_PRINTF("GetFrameData: I2C write failed with HAL error\r\n");
        return error;
    }
    
    // Small delay after clearing status before reading frame
    HAL_Delay(1);
    
    // Read the frame data (832 words from address 0x0400)
    error = MLX90640_I2CRead(slaveAddr, 0x0400, 832, frameData); 
    if(error != 0)
    {
        MLX90640_DRIVER_PRINTF("GetFrameData: Frame data read failed, error=%d\r\n", error);
        return error;
    }
    
    MLX90640_DRIVER_PRINTF("GetFrameData: Frame data read successfully\r\n");
    ...
    MLX90640_DRIVER_PRINTF("GetFrameData: Final statusReg=0x%04X, subpage=%d\r\n", statusRegister, frameData[833]);
